...
namespace Generic.Maths
{
    public static class LinearDistribution
    {
        ...
        public static IEnumerator<T> ApplySlow<T>(IList<T> xs)
        {
            ...
            return ListUtil.Repeat(() => (int) (xs.Count*Sample(random))).Distinct().Select(x => xs[x]).GetEnumerator();
        }

        public static IEnumerator<int> DistinctSampling(Random random, int count)
        {
            var indexes = ListUtil.FromTo(0, count);
            return ListUtil.Repeat(delegate
                {
        ...
        private static IList<int> TakeIntRewrite(this IList<int> xs, int amount)
        {
            ...
            return ListUtil.FromTo(fromTo.From, fromTo.From + amount - 1);
        }
        ...
        private static IList<int> DropIntRewrite(this IList<int> xs, int amount)
        {
            ...
            var fromTo = (FromToInt) xs;
            return ListUtil.FromTo(fromTo.From + amount, fromTo.To);
        }
        ...
        {
            return xs.TakeList(index).ConcatList(xs.Drop(index+1));
        }
